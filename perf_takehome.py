    ...
    def build_kernel(
        ...
        def build_ops(idx_chunk, val_chunk, regs, idx_ptr, val_ptr):
            ...
            for round_i in range(rounds):
                ...
                if use_level2_const and depth == 1 and rounds > 2:
                    ...
                    ops.append(("valu", [("+", cond_vec, cond_vec, one_vec)], "valu"))
                    ops.append(
                        (
                            "valu",
                            [
                ...
                if use_round_wrap:
                    if (round_i + 1) % wrap_period == 0:
                        ops.append(("valu", [("*", idx_chunk, idx_chunk, zero_vec)], "valu"))
                else:
                    ops.append(
        ...
        def schedule_wave(queues):
            ...
            while any(q.ready() for q in queues):
                ...
                need_addr = load_supply <= 4
                addr_budget = 2 if load_supply <= 2 else 1
                instr = {}
                used = set()
